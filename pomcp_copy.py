...
class POMCP():
    ...
    def __init__(self, generator, discount, exploration = 0.5, epsilon = 0.001, depth = 20):
        ...
        self.encoding_to_node: dict[str] = dict()
    ...
    def rollout(self, node: Node, state: tuple[int, int], depth: int)->float:
        ...
        if exit_found:
            ...
            rollout_node = Node(new_pos, new_obs, new_belief, node.id, random_action)
            if rollout_node.encoding in self.tree: 
                # if rollout node has already been encountered
                return reward + self.rollout(self.encoding_to_node[rollout_node.encoding])
            else:
                return reward + self.rollout(rollout_node, state, depth + 1)
    ...
    def simulate(self, state: tuple[int, int], node: Node, depth: int) -> float:
        ...
        if node.encoding not in self.tree:
            self.tree.add(node.encoding)
            self.encoding_to_node[node.encoding] = node
            ...
            for a in range(4):
                ...
                child_node = Node(new_pos, new_obs, new_belief, node.id, a)

                if child_node.encoding in self.tree: 
                    # if child node has already been encountered
                    node.children[a] = self.encoding_to_node[child_node.encoding]
                else:
