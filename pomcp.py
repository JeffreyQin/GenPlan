import random
import math
from generator import * #Figure this out later
...
class Node:
    ...
    """
    def __init__(self, obs, num_visited = 0, value = 0):
        self.num_visited:int = num_visited
        self.value:float = value
        self.obs:set[tuple[int, int]] = obs
        self.children:list[Node] = [] #empty for now


epsilon:float = 0.001 #cut off for the discount factor
tree:set[set[tuple[int, int]]] = set()




def rollout(node:Node, state: tuple[int, int], depth:int):
    """
    rollout function for exploring new actions/states
    """
    random_action = random.randint(0,3)
    new_state, obs, reward = generate(state, random_action)
    return reward + rollout(node.children[random_action], new_state, depth + 1)

    

def simulate(state:tuple[int, int], node:Node, depth:int) -> float:
    """
    simulate function for searching
    """
    if not node.obs in tree:
        tree.add(node.obs)
        for i in range(0,4):
            node.children.append(Node(node.obs, 0,0))
        ...
        new_state, obs, reward = generate(state, node ,action)

        reward = reward + simulate(new_state, node.children[a] ,depth + 1)
...
def best_action_index(root:Node) -> int:
    """
    Small helper function to find the action with the largest value given the node
    """
    action_values:list[float] = [root.children[0].value, root.children[1].value, root.children[2].value, root.children[3].value]
    best_action:int = action_values.index(max(action_values))
    return best_action

def search(root:Node) -> int:
    """
    ...
    best_action:int = best_action_index(root)
